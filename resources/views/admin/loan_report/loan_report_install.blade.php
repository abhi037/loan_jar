...
<script>
    $(document).ready(function() {
        var table = $('#loanReportTable').DataTable({ 
            processing: true,
            serverSide: true,
            ajax: {
                url: '{{ route("admin.loan.report.data.install") }}',
                data: function(d) {
            ...
            columns: [
                { data: 'id' },
                { data: 'loan_title', name: 'loan.title' },
                { data: 'title' },
                { data: 'description' },
                { data: 'amount' },
                { data: 'payment_method' },
        ...
        $('#filter').on('click', function() {
            table.draw(); // ✅ Now works
        });

        $.fn.dataTable.ext.errMode = 'throw'; // Optional
    });
</script>
...
<div class="modal fade" id="imageModal" tabindex="-1" aria-labelledby="imageModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content bg-dark position-relative">
      ...
      <!-- Close Button -->
      <div class="modal-header border-0" style="background:#247003 !important">
...
document.addEventListener('DOMContentLoaded', function () {
    document.body.addEventListener('click', function (e) {
        if (e.target.classList.contains('open-image-modal')) {
